...
function MainShop(props) {
    ...
    const loadProduct = useCallback(async (limitPage, sortName, sortPrice, offset, categoryId, keyword) => {
    ...
    }, [limitPage, sortName, sortPrice, offset, categoryId, keyword, loadProduct]);
    ...
    useEffect(() => {
        ...
        fetchCategory();
    }, [props.categoryId, props.brandId, sortPrice, sortName, limitPage, offset, keyword, loadProduct]);
                ...
                <div className="row">
                    {dataProduct && dataProduct.length > 0 &&
                        dataProduct.map((item, index) => {
                            return (
                                <ItemProduct
                                    key={item.id} // Chúng ta sử dụng id của mỗi sản phẩm làm key để đảm bảo tính duy nhất
                                    id={item.id}
                                    width={"255px"}
                                    height={"254px"}
