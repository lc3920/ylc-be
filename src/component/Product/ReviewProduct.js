...
import { ReplyReviewService, createNewReviewService, deleteReviewService, getAllReviewByProductIdService } from '../../services/userService';
...
function ReviewProduct(props) {
    ...
    const loadAllReview = useCallback(async () => {
        ...
        if (res && res.errCode === 0) {
            ...
            setInputValues(prevState => ({
                ...prevState, dataReview: res.data, countStar: {
                    star5: count5.length,
                    star4: count4.length,
                    star3: count3.length,
                    star2: count2.length,
    ...
    const openPreviewImage = (url) => {
        setInputValues(prevState => ({ ...prevState, imageReview: url, isOpen: true }));
    }

    const handleChooseStart = (number) => {
        setInputValues(prevState => ({ ...prevState, activeStar: number }));
    }
    ...
    const closeModal = () => {
        setInputValues(prevState => ({ ...prevState, isOpenModal: false, parentId: '' }));
    }

    const handleOpenModal = (id) => {
        setInputValues(prevState => ({ ...prevState, isOpenModal: true, parentId: id }));
    }

    const sendDataFromReViewModal = async (content) => {
        ...
        <div className="row">
                            ...
                            <ul className="list">
                                <li>
                                    <span>5
                                        ...
                                        <i className="fa fa-star" />
                                        <i className="fa fa-star" />
                                        <i className="fa fa-star" /> Có {inputValues.countStar.star5} lượt đánh giá</span>
                                </li>
                                <li>
                                    <span>4
                                        <i className="fa fa-star" />
                                        <i className="fa fa-star" />
                                        <i className="fa fa-star" />
                                        <i className="fa fa-star" /> Có {inputValues.countStar.star4} lượt đánh giá</span>
                                </li>
                                <li>
                                    <span>3
                                        <i className="fa fa-star" />
                                        <i className="fa fa-star" />
                                        <i className="fa fa-star" /> Có {inputValues.countStar.star3} lượt đánh giá</span>
                                </li>
                                <li>
                                    <span>2
                                        <i className="fa fa-star" />
                                        <i className="fa fa-star" /> Có {inputValues.countStar.star2} lượt đánh giá</span>
                                </li>
                                <li>
                                    <span>1
                                        <i className="fa fa-star" /> Có {inputValues.countStar.star1} lượt đánh giá</span>
                                </li>
                            </ul>
                        </div>
                    </div>
                                    ...
                                    <div key={index} className="review_item">
                                        <div className="media">
                                            <div className="d-flex">
                                                <img className="img-avatar" src={item.user.image} alt={name} />
                                            </div>
                                            <div className="media-body">
                                                ...
                                                {inputValues.user && inputValues.user.roleId === "R1" &&
                                                    <span style={{ cursor: 'pointer' }} onClick={() => handleOpenModal(item.id)} className="reply_btn">Phản hồi</span>
                                                }
                                            </div>
                                        </div>
                                        <div style={{ marginLeft: '88px' }}>
                                            ...
                                            {item.image &&
                                                <img onClick={() => openPreviewImage(item.image)} className="img-cmt" src={item.image} alt="" />
                                            }
                                            {item.childComment && item.childComment.length > 0 &&
                                                item.childComment.map((child, childIndex) => (
                                                    <div key={childIndex} className="box-reply">
                                                        <span>Phản hồi của người bán</span>
                                                        <p>{child.content}</p>
                                                        {inputValues.user && inputValues.user.roleId === "R1" &&
                                                            <button onClick={() => handleDeleteReply(child.id)} className="btn-delete-reply" type="button">Xóa</button>
                                                        }
                                                    </div>
                                                ))
                                            }
            ...
            </div>
            {
                ...
                <Lightbox mainSrc={inputValues.imageReview}
                    onCloseRequest={() => setInputValues(prevState => ({ ...prevState, isOpen: false, imageReview: '' }))}
                />
            }
            <ReviewModal
