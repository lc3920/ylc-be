              ...
              <div className="single-post">
                ...
                <div className="blog_details">
                  ...
                  <ul className="blog-info-link mt-3 mb-4">
                    <li><span><i className="ti-user" /></span> {dataBlog.userData && dataBlog.userData.firstName + " " + dataBlog.userData.lastName}</li>
                    <li><span><i className="ti-comments" /></span> {dataComment.length} Bình luận</li>
                  </ul>
                ...
                {dataComment && dataComment.length > 0 &&
                  dataComment.map((item, index) => {
                    if (item.user) {
                      ...
                      return (
                        <CommentBlog img={item.user.image} name={name} content={item.content} key={index} date={moment(item.createdAt).fromNow()} />
                      );
                    }
                    return null; // Returning null when there's no item.user
                  })
                }
              </div>
